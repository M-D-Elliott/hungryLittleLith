...
function deleteScore(form) {
    ...
    const scoreID = form.querySelector('.deleteScoreID').value;
    // const playerUsername = form.querySelector('.deleteScorePlayerName').value;
    const csrfToken = form.querySelector('.csrf').value;
    ...
    fetch(baseUrl + "/api/score/" + scoreID + "/delete", config)
        .then(function (response) {
            if (response.ok) {
                response.json().then(function (scoreObject) {
                    const scoreListing = form.parentNode.parentNode;

                    const scoreTable = scoreListing.parentNode;

                    scoreTable.removeChild(scoreListing);

                    const replaysListing = document.querySelector("#collapseReplays" + scoreID);

                    replaysListing.parentNode.removeChild(replaysListing);
        ...
        .catch(function () {
            const errorField = form.parentNode.parentNode.parentNode.querySelector(".scoreAddErrors");
            errorField.innerText = "Could not delete score.";
        });
    return false;
}

function approveScore(form) {

    const scoreID = form.querySelector('.approveScoreID').value;
    const playerUsername = document.querySelector('#currentPlayerUsername').value;
    const csrfToken = form.querySelector('.csrf').value;
    ...
    fetch(baseUrl + "/api/score/approve", config)
        .then(function (response) {
            if (response.ok) {
                response.json().then(function (scoreObject) {
                    const scoreListing = form.parentNode.parentNode;
